...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        if (retval == -1) { /* error */
            ...
            /* no fd response before timer expired */
        ...
        for (fd_index = 0; fd_index < max_fd; fd_index++) {
            ...
            if (retval > 0) 
                printf("%.*s", retval, buffer); /* read data from pipe */          
            else if (retval < 0)
                perror("pipe read()");          /* error */
            else {
                /* write fd closed */
                close(fd_index);                /* close read fd */
                FD_CLR(fd_index, &arfds);       /* remove fd from watch list */
            }
        }
    }
